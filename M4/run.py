...
def perform_evaluation(model_4, model_1, model_2, model_3, builder, eval_steps, ckpt, strategy, topology):
  """Perform evaluation."""
  ...
  with strategy.scope():
    ...
    checkpoint = tf.train.Checkpoint(model=model_4, global_step=tf.Variable(0, dtype=tf.int64))
  ...
  def single_step(features, labels):
    ...
    _, supervised_head_outputs = model_4(rep2, training=False)
    ...
    reg_loss = model_lib.add_weight_decay(model_4, adjust_per_optimizer=True)
...
# baray Module aval
  ...
  with strategy.scope():
    ...
    model_4 = model_lib.Model(num_classes)

  if FLAGS.mode == 'eval':
    for ckpt in tf.train.checkpoints_iterator(FLAGS.model_dir, min_interval_secs=15):
      result = perform_evaluation(model_4, model_1, model_2, model_3, builder, eval_steps, ckpt, strategy, topology)
    ...
    with strategy.scope():
      ...
      optimizer_4 = model_lib.build_optimizer(learning_rate)
    ...
    def single_step(features, labels):
      ...
      with tf.GradientTape() as tape:
        should_record = tf.equal((optimizer_4.iterations + 1) % steps_per_loop_3, 0)
        with tf.summary.record_if(should_record):
          # Only log augmented images for the first tower.
          tf.summary.image('image', features[:, :, :, :3], step=optimizer_4.iterations + 1)

        rep = model_1(features, training=False)
        _,b = model_2(rep, training=False)
        projection_head_outputs, supervised_head_outputs = model_4(b, training=True)
        flops(model_4)
        loss = None
        if projection_head_outputs is not None:
          outputs = projection_head_outputs
        ...
        weight_decay = model_lib.add_weight_decay(model_4, adjust_per_optimizer=True)
        weight_decay_metric.update_state(weight_decay)
        loss += weight_decay
        total_loss_metric.update_state(loss)
        ...
        #   logging.info(var.name)
        grads = tape.gradient(loss, model_4.trainable_variables)
        optimizer_4.apply_gradients(zip(grads, model_4.trainable_variables))
  ...
  # baray Module charom
    ...
    with strategy.scope():
      ...
      global_step = optimizer_4.iterations
      ...
      while cur_step_4 < train_steps_4:
        ...
        with summary_writer.as_default():
          train_multiple_steps(iterator)
          cur_step_4 = global_step.numpy()
          checkpoint_manager_4.save(cur_step_4)
          ...
          if FLAGS.mode == 'train_then_eval':
            perform_evaluation(model_4, model_1, model_2, model_3, builder, eval_steps,
                        checkpoint_manager_4.latest_checkpoint, strategy,topology)
        for metric in all_metrics:
          metric.reset_states()
