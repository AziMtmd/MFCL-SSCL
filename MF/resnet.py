...
class BatchNormRelu(tf.keras.layers.Layer):  # pylint: disable=missing-docstring
  ...
  def __init__(self,
    ...
    if FLAGS.global_bn:
      ...
      # Batch normalization layers with fused=True only support 4D input
      # tensors.
      self.bn = tfa.layers.BatchNormalization(
          axis=axis,
          momentum=FLAGS.batch_norm_decay,
          epsilon=BATCH_NORM_EPSILON,
          center=center,
...
class GNormRelu(tf.keras.layers.Layer):  # pylint: disable=missing-docstring
  ...
  def __init__(self,
               ...
               data_format='channels_last',
               **kwargs):
    super(GNormRelu, self).__init__(**kwargs)
    self.relu = relu
    if init_zero:
      gamma_initializer = tf.zeros_initializer()
    else:
...
class GNNRelu(tf.keras.layers.Layer):  # pylint: disable=missing-docstring
  ...
  def __init__(self,
               ...
               data_format='channels_last',
               **kwargs):
    super(GNNRelu, self).__init__(**kwargs)
    self.relu = relu
    if init_zero:
      gamma_initializer = tf.zeros_initializer()
    else:
...
class ResnetShort(tf.keras.layers.Layer):  # pylint: disable=missing-docstring
  ...
  def __init__(self,
               ...
               dropblock_size=None,
               **kwargs):
    super(ResnetShort, self).__init__(**kwargs)
    self.data_format = data_format
    if dropblock_keep_probs is None:
      dropblock_keep_probs = [None] * 4
    if not isinstance(dropblock_keep_probs,
  ...
  def call(self, inputs, training):
    ...
    inputs = tf.identity(inputs, 'final_avg_pool')
    print('000000000000', inputs.shape)
    return inputs
