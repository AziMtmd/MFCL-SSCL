...
flags.DEFINE_integer('train_epochs2', 1, 'Number of epochs to train for.')
...
def perform_evaluation(model, model2, builder, eval_steps, ckpt, strategy, topology):
  """Perform evaluation."""
  ...
  # save(model2, global_step=result['global_step'])
  a=model.weights  
  b=[0]*len(a)
  for i in range(len(a)-1):
    b[i]=a[i].numpy()
    numpy.save('m'+str(i)+'.npy', b[i], allow_pickle=False)

  a2=model2.weights
  b2=[0]*len(a2)
  for i in range(len(a2)-1):
    b2[i]=a2[i].numpy()
    numpy.save('p'+str(i)+'.npy', b2[i], allow_pickle=False)
...
def main(argv):
  ...
  train_steps2 = model_lib.get_train_steps(num_train_examples) 
  epoch_steps2 = int(round(num_train_examples / FLAGS.train_batch_size))
  logging.info('# train examples M1: %d', num_train_examples)
  logging.info('# train_steps M1: %d', train_steps2)
  logging.info('# epoch_steps M1: %d', epoch_steps2)
  logging.info('# eval examples M1: %d', num_eval_examples)
  logging.info('# eval steps M1: %d', eval_steps)
  checkpoint_steps2 = (FLAGS.checkpoint_steps or (FLAGS.checkpoint_epochs * epoch_steps2))

  train_steps3 = model_lib.get_train_steps(num_train_examples) 
  epoch_steps3 = int(round(num_train_examples / FLAGS.train_batch_size))
  logging.info('# train examples M1: %d', num_train_examples)
  logging.info('# train_steps M1: %d', train_steps3)
  logging.info('# epoch_steps M1: %d', epoch_steps3)
  logging.info('# eval examples M1: %d', num_eval_examples)
  logging.info('# eval steps M1: %d', eval_steps)
  checkpoint_steps3 = (FLAGS.checkpoint_steps or (FLAGS.checkpoint_epochs * epoch_steps3))

  with strategy.scope():
    model = model_lib.Model(num_classes)
    model2 = model_lib.Model2(num_classes)
    model3 = model_lib.Model3(num_classes)
  ...
  if FLAGS.mode == 'eval':
    ...
    with strategy.scope():
      ...
      optimizer2 = model_lib.build_optimizer(learning_rate)
      optimizer3 = model_lib.build_optimizer(learning_rate)
      ...
      checkpoint_manager2 = try_restore_from_checkpoint(model2, optimizer2.iterations, optimizer2)
      checkpoint_manager = try_restore_from_checkpoint(model, optimizer.iterations, optimizer)
      checkpoint_manager3 = try_restore_from_checkpoint(model3, optimizer3.iterations, optimizer3)
    ...
    def single_step2(features, labels):
      with tf.GradientTape() as tape:
        should_record = tf.equal((optimizer2.iterations + 1) % checkpoint_steps2, 0)
        with tf.summary.record_if(should_record):
          # Only log augmented images for the first tower.
          tf.summary.image('image', features[:, :, :, :3], step=optimizer2.iterations + 1)

        # rep = model_1(features, training=False)
        projection_head_outputs, supervised_head_outputs = model2(features, training=True)
        flops(model2)
        loss = None
        if projection_head_outputs is not None:
          outputs = projection_head_outputs
        ...
        if supervised_head_outputs is not None:
          ...
          if loss is None:
            ...
            loss += sup_loss
          metrics.update_finetune_metrics_train(supervised_loss_metric,supervised_acc_metric, sup_loss,l, outputs)
        weight_decay = model_lib.add_weight_decay(model2, adjust_per_optimizer=True)
        weight_decay_metric.update_state(weight_decay)
        loss += weight_decay
        total_loss_metric.update_state(loss)
        # The default behavior of `apply_gradients` is to sum gradients from all
        ...
        # print('****************************for the third module****************************')
        # model_summary(model)
        logging.info('Trainable variables 2:')
        print('%%%%%%OUPUT%%%%%%')
    ...
    def single_step3(features, labels):
      with tf.GradientTape() as tape:
        should_record = tf.equal((optimizer3.iterations + 1) % checkpoint_steps3, 0)
        with tf.summary.record_if(should_record):
          # Only log augmented images for the first tower.
          tf.summary.image('image', features[:, :, :, :3], step=optimizer3.iterations + 1)

        # rep = model_1(features, training=False)
        projection_head_outputs, supervised_head_outputs = model3(features, training=True)
        flops(model3)
        loss = None
        if projection_head_outputs is not None:
          outputs = projection_head_outputs
        ...
        if supervised_head_outputs is not None:
          ...
          if loss is None:
            ...
            loss += sup_loss
          metrics.update_finetune_metrics_train(supervised_loss_metric,supervised_acc_metric, sup_loss,l, outputs)
        weight_decay = model_lib.add_weight_decay(model3, adjust_per_optimizer=True)
        weight_decay_metric.update_state(weight_decay)
        loss += weight_decay
        total_loss_metric.update_state(loss)
        # The default behavior of `apply_gradients` is to sum gradients from all
    ...
    with strategy.scope():
      ...
      def train_multiple_steps(iterator):
        # `tf.range` is needed so that this runs in a `tf.while_loop` and is not unrolled.
        for _ in tf.range(checkpoint_steps2):
          # Drop the "while" prefix created by tf.while_loop which otherwise gets prefixed to every variable name. 
          # This does not affect training but does affect the checkpoint conversion script. TODO(b/161712658): Remove this.
          with tf.name_scope(''):
            images, labels = next(iterator)
      ...
      global_step2 = optimizer2.iterations
      cur_step2 = global_step2.numpy()
      iterator = iter(ds)
      while cur_step2 < train_steps2:
        # Calls to tf.summary.xyz lookup the summary writer resource which is
        # set by the summary writer's context manager.
        with summary_writer.as_default():
          train_multiple_steps(iterator)
          cur_step2 = global_step2.numpy()
          checkpoint_manager2.save(cur_step2)
          logging.info('Completed: %d / %d steps', cur_step2, train_steps2)
          metrics.log_and_write_metrics_to_summary(all_metrics, cur_step2)
          tf.summary.scalar('learning_rate',learning_rate(tf.cast(global_step2, dtype=tf.float32)),global_step2)
          summary_writer.flush()
        for metric in all_metrics:
          metric.reset_states()
    ...
    with strategy.scope():
      ...
      def train_multiple_steps(iterator):
        # `tf.range` is needed so that this runs in a `tf.while_loop` and is not unrolled.
        for _ in tf.range(checkpoint_steps3):
          # Drop the "while" prefix created by tf.while_loop which otherwise gets prefixed to every variable name. 
          # This does not affect training but does affect the checkpoint conversion script. TODO(b/161712658): Remove this.
          with tf.name_scope(''):
            images, labels = next(iterator)
      ...
      global_step3 = optimizer3.iterations
      cur_step_3 = global_step3.numpy()
      iterator = iter(ds)
      while cur_step_3 < train_steps3:
        # Calls to tf.summary.xyz lookup the summary writer resource which is
        # set by the summary writer's context manager.
        with summary_writer.as_default():
          train_multiple_steps(iterator)
          ...
          logging.info('Completed: %d / %d steps', cur_step_3, train_steps3)
          metrics.log_and_write_metrics_to_summary(all_metrics, cur_step_3)
          tf.summary.scalar('learning_rate',learning_rate(tf.cast(global_step3, dtype=tf.float32)),global_step3)
          summary_writer.flush()
        for metric in all_metrics:
          metric.reset_states()
      logging.info('Training 1 complete...')
    ...
    if FLAGS.mode == 'train_then_eval':
      perform_evaluation(model, model2, builder, eval_steps, checkpoint_manager.latest_checkpoint, strategy,topology)
