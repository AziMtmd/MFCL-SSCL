...
def perform_evaluation(model, model_1, model_2, model_3, builder, eval_steps, ckpt_1, ckpt_2, ckpt_3, ckpt, strategy, topology):
  """Perform evaluation."""
  ...
  with strategy.scope():
    ...
    # Restore checkpoint.
    logging.info('Restoring from %s', ckpt_1); logging.info('Restoring from %s', ckpt_2); logging.info('Restoring from %s', ckpt_3)
    logging.info('Restoring from %s', ckpt)
    # checkpoint = tf.train.Checkpoint(model=model_1, global_step=tf.Variable(0, dtype=tf.int64))
    # checkpoint.restore(ckpt_1).expect_partial()
    # checkpoint = tf.train.Checkpoint(model=model_2, global_step=tf.Variable(0, dtype=tf.int64))
    # checkpoint.restore(ckpt_2).expect_partial()
    # checkpoint = tf.train.Checkpoint(model=model_3, global_step=tf.Variable(0, dtype=tf.int64))
    # checkpoint.restore(ckpt_3).expect_partial()
  ...
  if FLAGS.mode == 'eval':
    ...
    def single_step_1(features, labels):
      ...
      with tf.GradientTape() as tape:
        ...
        hdd, fea , conv= model_1(features, training=True)
        # flops(model_1)
        loss = None
        if hdd is not None:
          outputs = hdd          
    ...
    def single_step_2(features, labels):
      ...
      with tf.GradientTape() as tape:
        ...
        rep = model_1(features, training=False)
        projection_head_outputs, supervised_head_outputs, conv = model_2(rep, training=True)
        # flops(model_2)
        loss = None
    ...
    def single_step_3(features, labels):
      ...
      with tf.GradientTape() as tape:
        ...
        projection_head_outputs, supervised_head_outputs, conv = model_3(b, training=True)
        # flops(model_3)
        loss = None
        if projection_head_outputs is not None:
          outputs = projection_head_outputs
...
# M_1
    with strategy.scope():
      ...
      steps_per_loop_2=steps_per_loop_1
      steps_per_loop_3=steps_per_loop_1
      checkpoint_steps=steps_per_loop_1
      print('steps_per_loop_1', steps_per_loop_1)
      iterator = iter(ds)
      global_step = optimizer_1.iterations; global_step2 = optimizer_2.iterations; global_step3 = optimizer_3.iterations
      global_step4 = optimizer.iterations
      cur_step_1 = global_step.numpy()
      while cur_step_1 < train_steps_1:
        with summary_writer.as_default():
          train_multiple_steps(iterator)
          cur_step_1 = global_step.numpy();cur_step_2 = global_step2.numpy(); cur_step_3 = global_step3.numpy()
          cur_step_4 = global_step4.numpy()
          checkpoint_manager_1.save(cur_step_1);checkpoint_manager_2.save(cur_step_2);checkpoint_manager_3.save(cur_step_3)
...
#       logging.info('Training 4 complete...')
    ...
    if FLAGS.mode == 'train_then_eval':
      perform_evaluation(model, model_1, model_2, model_3, builder, eval_steps, checkpoint_manager_1.latest_checkpoint, checkpoint_manager_2.latest_checkpoint,
                        checkpoint_manager_3.latest_checkpoint, checkpoint_manager.latest_checkpoint, strategy,topology)
