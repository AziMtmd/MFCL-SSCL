  ...
  if FLAGS.mode == 'eval':
    ...
    def single_step_1(features, labels):
      with tf.GradientTape() as tape:
        ...
        projection_head_outputs, supervised_head_outputs = model_1(features, training=True)
        #flops(model_1)
        loss = None
        supervised_head_outputs = tf.concat([supervised_head_outputs, supervised_head_outputs], 1)
        if projection_head_outputs is not None:
          outputs = projection_head_outputs
          unsup_loss = obj_lib.add_usupervised_loss(supervised_head_outputs, outputs)
        if loss is None:
          loss = unsup_loss
        else:
          loss += unsup_loss
