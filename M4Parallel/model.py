...
class Module_1(tf.keras.models.Model):
  ...
  def __call__(self, inputs, training):
    ...
    hiddens1 = self.resnet_module_1(features, training=FLAGS.module1_train)
    if FLAGS.module1_train==True:
      hiddens2 = self._decoder_1(hiddens1, training=FLAGS.module1_train)
      return hiddens2, features, hiddens1
    else:
      return hiddens1
...
class Module_2(tf.keras.models.Model):
  ...
  def __call__(self, inputs, training):
    ...
    if FLAGS.module2_train==True:
      projection_head_outputs, supervised_head_inputs = self._projection_head(hiddens, training)
      return projection_head_outputs, supervised_head_inputs, conv
    else:
      return conv      
