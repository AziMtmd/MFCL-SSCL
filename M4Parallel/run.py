...
flags.DEFINE_integer('m3_epoch', 150, 'Number of epochs to train for.')
  ...
  if FLAGS.mode == 'eval':
    ...
    def single_step_1(features, labels):
      with tf.GradientTape() as tape:
        ...
        hdd, fea, hdd2 = model_1(features, training=True)
        # flops(model_1)
        loss = None
        if hdd is not None:
          outputs = hdd          
      ...
      with tf.GradientTape() as tape2:
        ...
        # rep = model_1(features, training=False)
        projection_head_outputs, supervised_head_outputs, conv = model_2(hdd2, training=True)
        # flops(model_2)
        loss = None
        ...
        #   logging.info(var.name)
        grads = tape2.gradient(loss, model_2.trainable_variables)
      ...
      with tf.GradientTape() as tape3:
        ...
        projection_head_outputs2, supervised_head_outputs2, conv2 = model_3(conv, training=True)
        # flops(model_3)
        loss = None
        if projection_head_outputs2 is not None:
          outputs = projection_head_outputs2
          con_loss, logits_con, labels_con = obj_lib.add_contrastive_loss(
              outputs,hidden_norm=FLAGS.hidden_norm,temperature=FLAGS.temperature,strategy=strategy)
          if loss is None:
            loss = con_loss
        ...
        #   logging.info(var.name)
        grads = tape3.gradient(loss, model_3.trainable_variables)
      ...
      with tf.GradientTape() as tape4:
        ...
        projection_head_outputs3, supervised_head_outputs3 = model(conv2, training=True)
        #flops(model)
        loss = None
        if projection_head_outputs3 is not None:
          outputs = projection_head_outputs3
          con_loss, logits_con, labels_con = obj_lib.add_contrastive_loss(
              outputs,hidden_norm=FLAGS.hidden_norm,temperature=FLAGS.temperature,strategy=strategy)
          if loss is None:
            loss = con_loss
        ...
        if supervised_head_outputs3 is not None:
          outputs = supervised_head_outputs3
          l = labels['labels']
        ...
        #for var in model.trainable_variables:
         # logging.info(var.name)
        grads = tape4.gradient(loss, model.trainable_variables)
    ...
    with strategy.scope():
      ...
      while cur_step_1 < train_steps_1:
        with summary_writer.as_default():
          ...
          cur_step_1 = global_step.numpy()
          checkpoint_manager_1.save(cur_step_1);checkpoint_manager_2.save(cur_step_1);checkpoint_manager_3.save(cur_step_1)
